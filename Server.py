    ...
    def Loudness_Encoding(self,videofilename, tempfolder):
        try:
            ...
            ffArg_with_LUFS = ('ffmpeg\\bin\\ffmpeg.exe -y -hide_banner -i ' +
                ...
                ':measured_I=%s:measured_TP=%s:measured_LRA=%s:measured_thresh=%s:offset=%s' % (measured['input_i'], measured['input_tp'], measured['input_lra'], measured['input_thresh'], measured['target_offset'])+
...
if __name__ == '__main__':
    ...
    while not exit:
        ...
        if r == 'q':
            ...
            time.sleep(5)
            proc=proc_start()
        else: print ('invalid input: {}. Press"q" for quit or "rest" for restart'.format(r))
